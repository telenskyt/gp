...
#'
#' @details
#' The two-stage fitting method is a special optimization procedure (Telenský et al 2026) where the optimization is split into two stages:
#'
		...
		if (!two.stage) {
			...
			if (2 %in% stages) {
				...
				if (any(gp$hyperpar$low[gp$hyperpar$hyperpar == "sigma2" & gp$hyperpar$component %in% staged_components] - stage1low < sqrt(.Machine$double.eps)))
					stop("gpFit(): trying to proceed with stage 2, but some sigma2 components seem to be already restricted to levels for stage 1")
