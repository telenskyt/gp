...
# same as compareNA, but with NULL, and for non-vector variables
...
# jako compareNA, ale s toleranci
compareNAtol <- function(v1, v2, tol=1e-12) {
	same <- (abs(v1 - v2) < tol) | (is.na(v1) & is.na(v2))
	same[is.na(same)] <- FALSE
	return(same)
}


# good help for constructing data.frame's
null2na <- function (x) if (is.null(x)) NA else x
na2zero <- function (x) if (is.na(x)) 0 else x
